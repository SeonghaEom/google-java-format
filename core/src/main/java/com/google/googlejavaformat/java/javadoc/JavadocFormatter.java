 ...
 *
 * <p>
 * This stateless class reads tokens from the stateful lexer and translates them
 * to "requests" and "writes" to the stateful writer. It also munges tokens into
 * "standardized" forms. Finally, it performs postprocessing to convert the
 * written Javadoc to a one-liner if possible or to leave a single blank line if
 * it's empty.
 */
public final class JavadocFormatter {
   ...
   * The output will start and end with the same characters.
   */
  ...
  /*
   * TODO(cpovirk): Is this really the right location for the standardize*
   * methods? Maybe the lexer should include them as part of its own
   * postprocessing? Or even the writer could make sense.
   */
  ...
  /**
   * Returns the given string or a one-line version of it (e.g., "∕✱✱ Tests for
   * foos. ✱∕") if it fits on one line.
   */
